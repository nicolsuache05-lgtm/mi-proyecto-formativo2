...
<div class="card">
  ...
  @if(session('flash_error'))
    <div class="flash-msg flash-error">{{ session('flash_error') }}</div>
  @endif
  ...
  <form action="{{ route('registro.post') }}" method="POST">
    ...
    <div class="form-row">
      <div class="field-wrap">
        ...
        <div class="input-row"><input name="nombre" type="text" placeholder="Alejandra" required value="{{ old('nombre') }}"></div>
      </div>
      <div class="field-wrap">
        <label class="field-label">Apellido</label>
        <div class="input-row"><input name="apellido" type="text" placeholder="García" required value="{{ old('apellido') }}"></div>
      </div>
    </div>

    <div class="field-wrap">
      <label class="field-label">Correo electrónico</label>
      <div class="input-row"><input name="correo" type="email" placeholder="correo@ejemplo.com" required value="{{ old('correo') }}"></div>
    </div>

    <div class="field-wrap">
      <label class="field-label">Teléfono</label>
      <div class="input-row"><input name="telefono" type="tel" placeholder="+57 300 000 0000" value="{{ old('telefono') }}"></div>
    </div>
